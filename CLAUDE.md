...
Rilo Analysis 是一个宠物综合体经营测算应用，专门用于宠物行业的财务分析和经营预测。该应用提供完整的财务建模功能，包括四大收入板块测算、成本分析和投资回报计算，适用于路演、物业谈判和投资决策。
...
### Users
- 主要用户是需要快速完成经营测算、方案推演、路演表达、物业谈判或投资判断的人。
- 他们使用产品时通常处于“需要快速得出判断，但又必须信任结果”的情境。
- 这不是纯展示型页面，也不是后台管理系统；它首先是一个**可信、可操作、可复核的测算工作台**。

### Brand Personality
- 关键词：**有质感、文艺、冷静、专业**。
- 品牌气质需要在**温暖、专业、理性与感性**之间保持平衡。
- 用户希望它看起来不是冷冰冰的财务表，也不是轻浮的互联网营销页面，而是一种“有审美的专业工具”。
- 界面应传达的核心情绪：**可信、克制、沉静、略带温度**。

### Aesthetic Direction
- 视觉方向优先追求：**深色优先**，以更容易建立冷静、专业、有质感的第一印象。
- 允许适度文艺感与材料感，但不能牺牲信息层级和可读性。
- 参考方向：
  - `osmo.supply`：参考其**有趣但克制的灵动感**与清晰的信息层级。
  - `formless.xyz`：参考其**动效节奏**与**宝石感配色质感**，但需降动效强度。
- 反向约束：
  - **不要像花哨 SaaS landing page**。
  - 不要为了“酷”牺牲结构清晰度。
  - 不要把产品做成过度可爱、过度营销或过度金融投行风。

### Accessibility & Motion
- 动效策略：**弱动效**。
- 动效只用于层级切换、状态反馈与轻量强化，不用于炫技。
- 所有视觉决策优先保证：
  - 信息层级清晰
  - 文本可读
  - 对比明确
...
### Design Principles
1. **先可信，再惊艳**：视觉质感必须服务于“这是一个可以信的测算工具”。
2. **冷静但不冰冷**：整体基调克制、专业、偏深色，但保留适度温度与文艺感。
3. **信息层级第一**：任何配色、动效、装饰都不能压过结果层级与输入路径。
4. **中文表达优先**：界面面向决策与沟通，默认用中文表达概念；缩写只在公式或代码层出现。
5. **弱动效、强反馈**：少做花哨过场，多做明确、稳定、低打扰的状态反馈。
