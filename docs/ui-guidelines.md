...
### 1.1 页面结构（按新 prototype 锁定）
- **左侧导航**：页签/导航固定在左侧，负责页面切换。
- **右侧内容区**：当前页面的主内容全部放在右侧，按页面自身的信息层次展开。
- **禁止把 Inspector 常驻成右侧第二栏**：Inspector 保持 Drawer 形态，需要时再打开。

> 结论：整体壳层按“左侧导航 + 右侧内容”的双栏结构走，不再以页面内部 14/10 双栏作为主骨架。

### 1.2 Inspector / Drawer 职责边界
- **Inspector 保持 Drawer**：继续沿用之前已有的 Drawer 方案，不改成常驻右栏。
- **术语/公式/明细**：放进 Drawer 或详情区，不抢主内容首屏。
- **主页面默认只放数字和图表**：不放结论性文案，不放建议性文案。
- **如果需要看单业务指标**：放到下方详情区，不占顶部关键指标位。
...
- [ ] 页面结构是否按新 prototype：左侧导航 + 右侧内容
- [ ] Inspector 是否继续保持 Drawer，而不是常驻右栏
- [ ] 顶部工具区是否先隐藏，但保留代码与回滚能力
- [ ] 配色是否跟随 prototype 色板，克制统一，不花哨
- [ ] 顶部关键指标是否只保留全局指标
- [ ] 单业务指标是否下沉到详情区，而不是挤在顶部
- [ ] 页面是否移除了结论/建议性文案，只保留指标数字

## 7. Code Review Checklist（改 UI 必过）

- [ ] 页面是否遵循“左侧导航 + 右侧内容”主结构
- [ ] Inspector 是否仍为 Drawer
